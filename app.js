...
function executeLogin(pin) {
  if (USER_CONFIGS[pin]) {
    ...
    // Load live updates from Google Sheet in background
    loadData();
  } else {
...
async function loadData() {
  try {
    ...
    if (sheetJson && sheetJson.teammates) {
      // Preserve dailySummary while updating live teammate metrics
      ...
      if (sheetJson.directorsReportDriveUrl) liveSheetData.directorsReportDriveUrl = sheetJson.directorsReportDriveUrl;
      if (sheetJson.teammatesGapDriveUrl) liveSheetData.teammatesGapDriveUrl = sheetJson.teammatesGapDriveUrl;
    ...
    setSafeText("lastUpdated", "Using Verified Day 13 Actuals (Offline Mode)");
  }
}
